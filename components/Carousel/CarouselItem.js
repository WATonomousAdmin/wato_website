...
const CarouselItem = ({ title, cta, blurb, body, image, selected, idx, onToggle }) => {
  ...
  return (
    <div className={`m-5 h-[90vh] lg:h-[70vh] ${/*"group"*/ ""} ${!active && selected !== -1 ? "w-[0px] opacity-0 overflow-hidden" : "max-lg:w-[40vw] lg:w-[30%] delay-1000"}  transition-all duration-500`}>
      <div className={`relative h-[64.25vh] lg:h-[50vh] transition-all ${active ? "-translate-x-1/2 delay-200 w-[200%]" : "w-[100%]"} duration-1000`}>
